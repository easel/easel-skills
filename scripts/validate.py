...
def validate_dev_skill_links() -> None:
    """Project discovery links under .agents/skills must point at skills/."""
    links_root = ROOT / ".agents" / "skills"
    if not links_root.is_dir():
        fail("missing .agents/skills (run scripts/sync-plugin-skills.sh)")
    expected = repo_skill_names()
    found: set[str] = set()
    for entry in sorted(links_root.iterdir()):
        name = entry.name
        found.add(name)
        if not entry.is_symlink():
            fail(f".agents/skills/{name} must be a symlink to ../../skills/{name}")
        target = entry.readlink()
        if target != Path(f"../../skills/{name}"):
            fail(
                f".agents/skills/{name} must resolve to ../../skills/{name}, "
                f"got {target}"
            )
        if not (ROOT / "skills" / name).is_dir():
            fail(f".agents/skills/{name} points at missing skills/{name}")
    if found != expected:
        ...
        fail(f".agents/skills links must match skills/ ({'; '.join(details)})")
